...
function audio_play(i) {
    if(i == "default") {
        var audio = new Audio("audio/default/saodi.mp3");
        audio.play();
    } else {
...
//点击时的效果
function animation() {
    $(".woodfish .click img").css("transform", "rotateZ(0deg)");
    setTimeout(() => {
        $(".woodfish .click img").css("transform", "rotateZ(-20deg)");
    }, 80);
    setTimeout(() => {
        $(".woodfish .click img").css("transform", "rotateZ(0deg)");
    }, 200);
...
function animation_text() {
    ...
    $(".woodfish").append("<div id=\"text_tips_" +randomNum_1+ "\" class=\"text_tips\">悼念+1</div>");
    setTimeout(() => {
    }, 5);
    setTimeout(() => {
